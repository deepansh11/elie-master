...
class _EditUserState extends State<EditUser> {
  ...
  getExpert(id) async {
    ...
    period.text =
        e['periods'].toString() == 'null' ? '' : e['periods'].toString();
    ...
    serviceablePin.text = e['serviceablePins']
        .toString()
        .substring(1, e['serviceablePins'].toString().length - 1);
    pinList = e['serviceablePins'];
    vehicle.text = e['vehicle'];
    expertise.text = e['serviceList']
        .toString()
        .substring(1, e['serviceList'].toString().length - 1);
    ...
    if (e['aadhar'] != null) {
      aadharImage1 =
          '$baseURL/get_single_expert_photo/${e['aadhar'][0].toString()}';
      aadharImage2 =
          '$baseURL/get_single_expert_photo/${e['aadhar'][1].toString()}';
    }
    ...
    for (var d in e['experience']) {
      apartmentImages.add(Padding(
        ...
        child: InkWell(
          onTap: () {
            js.context
                .callMethod('open', ['$baseURL/get_single_expert_photo/$d']);
          },
          child: ClipRRect(
  ...
  void initState() {
    ...
    anniversary.text = getItEditUser.anniversary.toString() == 'null'
        ? ''
        : getItEditUser.anniversary.toString();
    bloodGrp.text = getItEditUser.bloodGrp.toString() == 'null'
        ? ''
        : getItEditUser.bloodGrp.toString();
                          ...
                          : () async {
                              try {
                                ...
                                if (name.text.isNotEmpty &&
                                  ...
                                  if (isCustomer) {
                                    await Dio().put(
                                        '$baseURL/updateCustomerName/${phone.text}',
                                        queryParameters: {
                                          'updated': name.text
                                        });
                                    await Dio().put(
                                        '$baseURL/updateCustomerEmail/${phone.text}',
                                        queryParameters: {
                                          'updated': email.text
                                        });

                                    await Dio().put(
                                        '$baseURL/updateCustomerSex/${phone.text}',
                                        queryParameters: {
                                          'updated': gender.text
                                        });
                                    await Dio().put(
                                        '$baseURL/updateCustomerDob/${phone.text}',
                                        queryParameters: {'updated': dob.text});
                                    await Dio().put(
                                        '$baseURL/updateCustomerAnniversary/${phone.text}',
                                        queryParameters: {
                                          'updated': anniversary.text
                                        });
                                    await Dio().put(
                                        '$baseURL/updateCustomerBloodGroup/${phone.text}',
                                        queryParameters: {'updated': bloodGrp});

                                    if (image != null) {
                                      await Dio().post(
                                          '$baseURL/addCustomerImage/${phone.text}',
                                          data: image.bytes);
                                    }
                                    setState(() {
                                      loadingBtn = false;
                                    ...
                                    await Dio()
                                      ...
                                      'aadharNo': int.parse(aadhar.text.isEmpty
                                          ? '0'
                                          : aadhar.text),
                                      'panNo': pan.text,
  ...
  void showMultiSelectExpertise(BuildContext context) async {
    final initialItem = eList.map((e) => e.toString()).toList();
  ...
  void showMultiSelectPincodes(BuildContext context, bool isEmployee) async {
    final initialItem = pinList
        .map(
          (e) => e.toString(),
        )
        .toList();

    final item = isEmployee
        ? employeePinCodes
            .map(
              (e) => MultiSelectItem<String>(
                e.toUpperCase().trim().replaceAll(' ', ''),
